
#include <stdio.h>
#include <stdint.h>
#include <stdarg.h>
#include <stddef.h>
#include <setjmp.h>
#include "cmocka.h"
#include <string.h>
#include "ratp.h"

/* A test case that does nothing and succeeds. */

void recupAge(int* age){
    *age=(int)mock();
}

static int setup(void **state) {
    (void) state;
    printf("setUp");
 return 0;
}
static int teardown(void **state) {
    (void) state;
    printf("tearDown");
     return 0;
}
static void null_test_success(void **state) {
    (void) state;
}
static void testFF(void **state){
    will_return(recupAge,12);
    assert_float_equal(1.2, computePrice(13, 0), 0);
}
static void testFV(void **state){
    will_return(recupAge,12);
    assert_float_equal(2.4, computePrice(13, 1), 0);
}
static void testVF(void **state){
    will_return(recupAge,12);
    assert_float_equal(0.6, computePrice(11, 0), 0);
}
static void testVV(void **state){
    will_return(recupAge,12);
    assert_float_equal(1.2, computePrice(11, 1), 0);
}




int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(null_test_success),
        cmocka_unit_test(testFF),
        cmocka_unit_test(testFV),
        cmocka_unit_test(testVF),
        cmocka_unit_test_setup(testVV,setup),
    };
 //   cmocka_set_message_output(CM_OUTPUT_XML);
    return cmocka_run_group_tests_name("toto",tests, setup, teardown);
}
